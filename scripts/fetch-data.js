...
// Agrega os alertas de encomendas em UM alerta por tipo. Sem alertas de desconto (removidos a pedido).
// Também calcula o top 5 de produtos vendidos hoje (por receita), a partir das linhas das encomendas.
function computeShopifyMetrics(orders, timeZone) {
  ...
  const unfulfilled = [];
  const productTotals = {}; // nome -> { quantity, revenue }
...
// NOTA sobre "orçamento da campanha": tentei confirmar se a Flyweel expõe isto,
// mas a lista de métricas válidas que já vimos em testes anteriores não inclui
// nenhum campo de orçamento — só métricas de performance (spend, cliques, etc.),
// não configuração da campanha. Por agora não é possível mostrar essa coluna
// através desta ferramenta. Fica anotado para explorar uma via diferente (API
// direta da Meta) se for mesmo importante.
async function fetchFlyweelCampaigns(store) {
  ...
  try {
    ...
    const data = await res.json();
    console.log('DEBUG Flyweel resposta (today):', JSON.stringify(data).slice(0, 3000));
    ...
    // Alertas de performance: só campanhas ativas, com spend real, objetivo de vendas, ROAS < 1x, HOJE.
    // NOTA: como isto é só do dia de hoje, campanhas com pouco volume ainda podem
    // aparecer com ROAS 0 só por ainda não terem tido conversão nas últimas horas —
    // é um trade-off aceite para já, ao usarmos "hoje" como período por defeito.
